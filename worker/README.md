...
Create a worker-only token file with at least 32 characters and restrict it to the worker account. Keep conversation storage and credential storage on separate durable volumes.
...
Use a private container network. Do not publish port 3001. Run the image with a read-only root filesystem, `--cap-drop=ALL`, `--security-opt=no-new-privileges`, writable mounts only for the two state directories and `/tmp`, and no Docker socket or host-development mounts.
...
Pi resource loading is explicit and empty. The worker uses in-memory empty settings, an empty resource loader, no context files, no extensions, no packages, no skills, no prompts, no themes, and an empty tool allowlist. It never discovers a host Pi installation, settings, credentials, sessions, workspace, extensions, or skills.
